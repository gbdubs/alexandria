...
// tl1Data is one installation's TL1 workflow history, loaded from the tl1_*
// tables and joined with priced token usage from the attempts' transcripts.
type tl1Data struct {
...
func (c *Catalog) tl1Installations() ([]map[string]any, error) {
	return queryMaps(c.DB, `SELECT i.installation_id,i.project,i.repository,i.config_path,i.database_path,i.synced_at,
		...
		FROM tl1_installations i ORDER BY last_task_at DESC, tasks DESC`)
}

// loadTL1 reads an installation's history, keeping tasks created inside the
// window. Scope "current" keeps only tasks run with their flavor's current
// shape version (the live workflow definition).
func (c *Catalog) loadTL1(installationID string, window tl1Window) (*tl1Data, error) {
	scope := defaultString(window.Scope, "all")
	installations, err := queryMaps(c.DB, "SELECT * FROM tl1_installations WHERE installation_id=?", installationID)
	if err != nil {
		return nil, err
	}
	if len(installations) == 0 {
		return nil, fmt.Errorf("unknown TL1 installation %q", installationID)
	}
	data := &tl1Data{Installation: installations[0], Flavors: map[string]*tl1Flavor{}, TaskByID: map[string]*tl1Task{}, Candidates: map[string]map[string]any{}, Window: window, Scope: scope}
	...
	for _, row := range tasks {
		task := &tl1Task{ID: firstString(row["task_id"]), WorkspaceID: firstString(row["workspace_id"]), Flavor: firstString(row["flavor"]),
			...
			ClaimedAt: firstString(row["claimed_at"]), CompletedAt: firstString(row["completed_at"])}
	...
	ordered := make([]*tl1Task, 0, len(all))
	for _, row := range tasks {
		ordered = append(ordered, all[firstString(row["task_id"])])
	}
	...
	for _, task := range data.Tasks {
		used[task.CandidateID] = true
	}
	for _, row := range candidates {
		if id := firstString(row["candidate_id"]); used[id] {
	...
	inWindow := func(rows []map[string]any) []map[string]any {
		...
		for _, row := range rows {
			if task := firstString(row["task_id"], row["review_task_id"]); task != "" && data.TaskByID[task] == nil {
				continue
			}
			if task := firstString(row["task_id"], row["review_task_id"]); task == "" && !used[firstString(row["candidate_id"])] {
				continue
			}
	...
	for _, target := range []struct {
		...
		{"SELECT * FROM tl1_configuration_health WHERE installation_id=?", &data.Health},
	} {
		rows, err := queryMaps(c.DB, target.query, installationID)
		if err != nil {
			return nil, err
		}
		*target.into = inWindow(rows)
	}
	return data, nil
}
