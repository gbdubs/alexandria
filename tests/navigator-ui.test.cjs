...
test('navigator restores routes, library search, view, and query filters', async () => {
  ...
  try {
    ...
    await page.evaluate(() => window.pharosQueryTables.filterRepository('example/repo'));
  ...
  try {
    ...
    await page.route('http://navigator.test/**', route => {
      ...
      if (url.pathname === '/api/query/library' && route.request().method() === 'POST') return route.fulfill({ contentType: 'application/json', body: JSON.stringify({ rows: [{
        id: 'work-1', title: 'Past work', repository_name: 'pharos', source_kind: 'conductor',
        pr_count: 2, pr_numbers: '42,73', canonical_remote: 'git@github.com:acme/pharos.git',
        main_merge_commit: 'abcdef1234567890', main_merge_title: 'Merge feature into main', main_merge_method: 'merge', main_merge_url: 'https://github.com/acme/pharos/commit/abcdef1234567890',
        pr_details: JSON.stringify([{ number: 42, title: 'Improve archive search', url: 'https://github.com/acme/pharos/pull/42', host: 'github.com' }, { number: 73, title: null, url: null, host: 'github.com' }]),
      }], total: 1 }) });
    ...
    assert.equal(await titled.getAttribute('href'), 'https://github.com/acme/pharos/pull/42');
    assert.equal(await page.getByRole('link', { name: 'PR #73' }).getAttribute('href'), 'https://github.com/acme/pharos/pull/73');
    assert.equal(await titled.getAttribute('target'), '_blank');
    assert.equal(await page.getByRole('link', { name: 'Merged: Merge feature into main' }).getAttribute('href'), 'https://github.com/acme/pharos/commit/abcdef1234567890');
  } finally { await browser.close(); }
});
